...
export async function GET() {
  try {
    ...
    const [
      ...
      atItemsResult,
      // Card 2 source: jenis dari assets_clean yang belum dialokasikan
      ...
      // !inner = filter row item berdasarkan kondisi di tabel sj.
      supabase.from('surat_jalan_items')
        .select(`
        ...
        `)
        .eq('is_aktiva', true)
        .in('sj.status', ['submitted', 'completed'])
        .limit(50000),
    ...
    // Group items by SJ tanggal, count per hari. Lalu fill tanggal kosong.
    ...
    for (const it of (atItemsResult.data ?? [])) {
      ...
      if (tgl < monthStartIso || tgl > todayIso) continue
      dailyMap.set(tgl, (dailyMap.get(tgl) ?? 0) + 1)
    }

    // Fill semua tanggal di bulan berjalan (1 sampai hari ini) dengan 0
    // agar chart punya titik di setiap hari (tidak ada gap visual).
