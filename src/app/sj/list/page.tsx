...
import { useSJList, type SJListItem } from "@/hooks/useSJList";
    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
      <div className="bg-[#0d1117] border border-white/10 rounded-2xl w-full max-w-md p-6">
        ...
        <p className="text-xs text-white/50 mb-1">
          <span className="text-rose-400 font-semibold">{sj.no_sj}</span>
        </p>
        <p className="text-xs text-white/50 mb-4">
          {sj.tujuan?.kode} — {sj.tujuan?.nama} ({sj.items_count} item)
        </p>
        <p className="text-xs text-amber-400/80 mb-4">
          ⚠ Surat Jalan beserta semua item-nya akan dihapus permanen. Tindakan ini tidak bisa dibatalkan.
        </p>
...
export default function SJListPage() {
  ...
  const filtered = useMemo(() => {
    ...
    // Status filter
    if (statusFilter !== "all") {
      result = result.filter(sj => sj.status === statusFilter);
    }

    // Period filter
    if (periodFilter !== "all") {
      result = result.filter(sj => isInPeriod(sj.tanggal, periodFilter));
    }
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto px-6 py-5">
          ...
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-0.5 text-xs text-white/40">
                Kelola seluruh Surat Jalan yang sudah dibuat — edit, reschedule, atau hapus
              </p>
            </div>
            <Link href="/sj/buat"
          ...
          <div className="overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.03] shadow-xl shadow-black/30">
            ...
            {/* Toolbar — Filter Status + Periode + Search */}
            ...
            {/* Table Header */}
            <div className="grid grid-cols-[200px_100px_180px_80px_60px_100px_130px] gap-3 border-b border-white/[0.06] px-5 py-3 text-[10px] font-semibold uppercase tracking-widest text-white/25">
              <span>No. SJ</span>
              <span>Tanggal</span>
              <span>Tujuan</span>
              <span>Pembawa</span>
            ...
            {loading ? (
              ...
              <>
                <div className="divide-y divide-white/[0.04]">
                  {paginated.map(sj => (
                    <div key={sj.id} className="grid grid-cols-[200px_100px_180px_80px_60px_100px_130px] gap-3 items-center px-5 py-3 hover:bg-white/[0.02] transition-colors">

                      <span className="text-[11px] font-mono font-semibold text-cyan-400 truncate" title={sj.no_sj}>
                        {sj.no_sj}
                      </span>
                      ...
                      <span className="text-[11px] text-white/60 truncate" title={sj.pembawa}>
                        {sj.pembawa || "—"}
                      </span>
                      <span className="text-[11px] font-mono text-white/70 text-right">
                        {sj.items_count}
                      </span>
                      ...
                      <div className="flex items-center justify-center gap-0.5">
                        <button
                          onClick={() => router.push(`/sj/buat?edit=${sj.id}`)}
                          title="Edit"
                        ...
                        <button
                          onClick={() => setRescheduleTarget(sj)}
                          title="Reschedule"
